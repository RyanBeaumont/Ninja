    ...
    void Update()
    {
        ...
        if (quickTimeEvent.gameObject.activeInHierarchy)
        {
            ...
            if (Input.GetKeyDown(KeyCode.Space))
            {
                ...
                if(difference < quickTimeCritWindow) quickTimeMultiplier = 1.5f;
                else quickTimeMultiplier = 1 - (difference * 2);
                GameManager.Instance.ShowMessage($"{quickTimeMultiplier}X");
                quickTimeEvent.gameObject.SetActive(false);
                clock = 0f;
            }
    ...
    List<Combatant> SimulateNextTurns(int count)
{
    ...
    public void PlayerHit()
    {
        ...
        {
            if(t.alive){
            t.TakeDamage(activeCombatant,(int)pendingDamage * quickTimeMultiplier, pendingDamageType);
